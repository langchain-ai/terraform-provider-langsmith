...
func TestDeploymentCreateReadPathsAndPayload(t *testing.T) {
	...
	want := map[string]any{
		...
		"shareable":              true, "route_through_gateway": false,
	}
...
func TestDeploymentUpdateSeparatesPatchAndRevision(t *testing.T) {
	...
	if !ok || len(secrets) != 1 || secrets[0].(map[string]any)["value"] != "new-secret" {
...
func TestDeploymentReadRefreshesDesiredFieldsWithoutSecrets(t *testing.T) {
	...
	previous.SourceRevisionConfig.ImageURI = types.StringValue("stale-image")
	previous.SecretReferences = nil
	read, err := testDeploymentResource(server).read(context.Background(), testDeploymentID, previous)
	if err != nil {
		t.Fatal(err)
	}
	if read.SourceConfig.CustomURL.ValueString() != "orders.example.com" || read.SourceRevisionConfig.ImageURI.ValueString() != "registry.example.com/orders:v1" || len(read.SecretReferences) != 1 {
		t.Fatalf("server-readable fields not refreshed: %#v", read)
	}
...
func TestDeploymentRevisionPayloadDoesNotSendUnavailableSecrets(t *testing.T) {
	...
	payload := revisionPayload(model, false)
...
func TestDeploymentRevisionFailure(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"id":"` + testRevisionID + `","status":"DEPLOY_FAILED","status_message":"` + message + `"}`))
	}))
	defer server.Close()
	_, err := testDeploymentResource(server).waitForRevision(context.Background(), testDeploymentID, testRevisionID)
	if err == nil || !strings.Contains(err.Error(), "DEPLOY_FAILED") || !strings.Contains(err.Error(), message) {
...
func TestDeploymentUpdatePreservesAppliedRevisionAfterWaitFailure(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"id":"` + newRevisionID + `","status":"DEPLOY_FAILED","status_message":"` + message + `"}`))
	}))
	defer server.Close()
	state := testDeploymentModel()
	state.ID = types.StringValue(testDeploymentID)
	...
	if partial.LatestRevisionID.ValueString() != newRevisionID || partial.LatestRevisionStatus.ValueString() != "DEPLOY_FAILED" || partial.LatestRevisionStatusMessage.ValueString() != message {
		t.Fatalf("partial revision state = %#v", partial)
	}
	if partial.ActiveRevisionID.ValueString() != testRevisionID {
		t.Fatalf("active revision = %q", partial.ActiveRevisionID.ValueString())
	}
...
func testDeploymentModel() deploymentResourceModel {
	return deploymentResourceModel{
		...
		SecretReferences: []deploymentSecretReferenceModel{{Name: types.StringValue("DATABASE_URL"), SecretName: types.StringValue("orders"), SecretKey: types.StringValue("url")}}, Shareable: types.BoolValue(true), RouteThroughGateway: types.BoolValue(false),
	}
}

func deploymentResponseJSON(status string) string {
	return `{"id":"` + testDeploymentID + `","name":"orders","source":"external_docker","display_name":"Orders","source_config":{"custom_url":"orders.example.com","listener_id":"listener","listener_config":{"k8s_namespace":"agents"},"resource_spec":{"min_scale":1,"max_scale":3,"cpu":0.5,"memory_mb":1024,"labels":{"team":"agents"}}},"source_revision_config":{"image_uri":"registry.example.com/orders:v1"},"secret_references":[{"name":"DATABASE_URL","secret_name":"orders","secret_key":"url"}],"shareable":true,"route_through_gateway":false,"tenant_id":"aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa","created_at":"2025-01-01T00:00:00Z","updated_at":"2025-01-01T00:00:01Z","status":"` + status + `","latest_revision_id":"` + testRevisionID + `","active_revision_id":"` + testRevisionID + `","tracer_session_id":"bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb","url":"https://orders.example.com"}`
}
