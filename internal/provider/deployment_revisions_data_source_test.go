...
func TestDeploymentRevisionsDataSourceQueryAndMapping(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"resources":[{"id":"revision-id","created_at":"2025-01-01T00:00:00Z","updated_at":"2025-01-01T00:01:00Z","status":"DEPLOYED","status_message":null,"source":"image","source_revision_config":{"image_uri":"image:v1","tracked_packages":["agent"]}}],"offset":75}`))
	}))
	defer server.Close()
	...
	if model.ID != types.StringValue("revision-id") || model.Status != types.StringValue("DEPLOYED") || !model.StatusMessage.IsNull() || model.SourceRevisionConfig.ImageURI != types.StringValue("image:v1") || model.SourceRevisionConfig.TrackedPackages.Elements()[0] != types.StringValue("agent") {
